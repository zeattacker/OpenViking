...
async def vectorize_file(
    ...
    try:
        ...
        enqueued = True
        logger.info(f"[vectorize_file] Enqueued L2 for {file_path}, abstract={repr(summary[:60]) if summary else '(empty)'}")
...
def _parse_overview_summaries(overview: str) -> Dict[str, str]:
    """Extract per-file summaries from .overview.md content.

    Overview format (generated by SemanticProcessor):
        ### filename.md filename.md
        Summary text for this file...

        ### other.md other.md
        Summary text for the other file...

    Returns a dict mapping filename -> summary text.
    """
    ...
    for line in overview.split("\n"):
        ...
        if stripped.startswith("### ") and ".md" in stripped:
            ...
            if current_file and current_lines:
                summaries[current_file] = " ".join(current_lines).strip()
            # Parse new header: "### filename.md filename.md" or "### filename.md"
            header = stripped[4:].strip()
            parts = header.split()
            current_file = parts[0] if parts else None
            current_lines = []
        elif current_file and stripped:
            # Skip Quick Navigation / markdown links before per-file sections
...
async def index_resource(
    ...
    2. Scans files in the directory and vectorizes them, using per-file
       summaries extracted from .overview.md as abstracts when available.

    Infers context_type from the URI: memories/ and episodes/ -> "memory",
    skills/ -> "skill", everything else -> "resource".
    """
    ...
    try:
        if await viking_fs.exists(abstract_uri, ctx=ctx):
            ...
            logger.info(f"[index_resource] .abstract.md not found: {abstract_uri}")
    except Exception as e:
        logger.warning(f"[index_resource] Failed to read abstract: {abstract_uri}: {e}")
    ...
    try:
        if await viking_fs.exists(overview_uri, ctx=ctx):
            ...
            logger.info(f"[index_resource] .overview.md not found: {overview_uri}")
    except Exception as e:
        logger.warning(f"[index_resource] Failed to read overview: {overview_uri}: {e}")

    logger.info(
        f"[index_resource] {uri}: abstract={len(abstract)} chars, overview={len(overview)} chars"
    )
    ...
    # Extract per-file summaries from overview for L2 abstract population
    file_summaries = _parse_overview_summaries(overview)
    logger.info(f"[index_resource] {uri}: parsed {len(file_summaries)} summaries from overview")
    if file_summaries:
        logger.info(
            f"[index_resource] Parsed {len(file_summaries)} file summaries from overview for {uri}"
        )

    # 2. Index Files
    try:
        files = await viking_fs.ls(uri, ctx=ctx)
        logger.info(f"[index_resource] ls({uri}) returned {len(files)} items")
        for file_info in files:
            ...
            # Skip _archive directories — archived data should not be indexed
            ...
            if file_info.get("type") == "directory" or file_info.get("isDir"):
                ...
                await index_resource(subdir_uri, ctx)
                continue
            ...
            await vectorize_file(
                file_path=file_uri,
                summary_dict={"name": file_name, "summary": summary},
                parent_uri=uri,
                context_type=context_type,
                ctx=ctx,
            )
